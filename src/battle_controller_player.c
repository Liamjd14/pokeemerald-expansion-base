...
const u8 *BattleUI_GetTypeEffectivenessSymbol(enum BattlerId battler, enum Move move)
{
    ...
    struct ChooseMoveStruct *moveInfo = (struct ChooseMoveStruct *)(&gBattleResources->bufferA[battlerDef][4]);
    struct DamageContext ctx = {0};
    ctx.battlerAtk = battlerDef;
    ctx.battlerDef = battlerDef;
    ctx.move = moveInfo->moves[gMoveSelectionCursor[battlerDef]];
    ctx.moveType = CheckDynamicMoveType(GetBattlerMon(battlerDef), ctx.move, battlerDef, MON_IN_BATTLE);
    ctx.updateFlags = FALSE;
    ctx.weather = GetWeather();
    ctx.terrain = gFieldTimers.terrain;
    ctx.abilities[ctx.battlerDef] = GetBattlerAbility(battlerDef);
