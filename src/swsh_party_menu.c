...
static EWRAM_DATA struct MoveSlotNew sMoveSlots[MAX_MON_MOVES];
...
static bool8 TrySwitchInPokemon(void)
{
    ...
    if (gPartyMenu.action == PARTY_ACTION_CANT_SWITCH)
    {
        ...
        GetMonNickname(&gParties[B_TRAINER_PLAYER][GetPartyIdFromBattleSlot(gBattlerPartyIndexes[currBattler])], gStringVar1);
        StringExpandPlaceholders(gStringVar4, gText_PkmnCantSwitchOut);
        return FALSE;
    }
    gSelectedMonPartyId = GetPartyIdFromBattleSlot(slot);
    gPartyMenuUseExitCallback = TRUE;
    newSlot = GetPartyIdFromBattleSlot(gBattlerPartyIndexes[gBattlerInMenuId]);
