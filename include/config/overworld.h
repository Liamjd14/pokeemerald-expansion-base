...
#define OW_DEFOG_FIELD_MOVE             TEUE       // If enabled, Defog can be used as a Field Move as seen in DPPt.
...
#define B_FLAG_FOLLOWERS_DISABLED      FLAG_SYS_OW_DISABLE_FOLLOWER          // Enables / Disables followers by using a flag. Helpful to disable followers for a period of time.
...
#define OW_FLAG_PAUSE_TIME          FLAG_SYS_OW_PAUSE_TIME  // If this flag is set and OW_USE_FAKE_RTC is enabled, seconds on the in-game clock will not advance.
#define OW_FLAG_NO_ENCOUNTER        FLAG_SYS_OW_NO_ENCOUNTER  // If this flag is set, wild encounters will be disabled.
#define OW_FLAG_NO_TRAINER_SEE      FLAG_SYS_OW_NO_TRAINER_SEE  // If this flag is set, trainers will not battle the player unless they're talked to.
#define OW_FLAG_NO_COLLISION        FLAG_SYS_OW_NO_COLLISION  // If this flag is set, the player will be able to walk over tiles with collision. Mainly intended for debugging purposes.
#define OW_FLAG_POKE_RIDER          FLAG_SYS_OW_POKE_RIDER  // If this flag is set, the player will be able to use fly from the Pokenav Region Map and the Town Map key item by pressing 'R' on a city/location they are able to fly to.
...
#define OW_FLAG_MOVE_UNION_ROOM_CHECK    FLAG_SYS_OW_UNION_ROOM                  // If this flag is set, the game will only check if players are in the Union Room while healing Pokémon, and not when players enter the Pokémon Center. This speeds up the loading of the Pokémon Center. This is ignored if OW_UNION_DISABLE_CHECK is TRUE.
