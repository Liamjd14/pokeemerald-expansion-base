...
#define B_SMART_WILD_AI_FLAG        FLAG_SYS_SMART_WILD_AI_FLAG     // If not 0, you can set this flag in a script to enable smart wild pokemon
...
#define B_FLAG_NO_RUNNING           FLAG_SYS_NO_RUNNING     // If this flag is set, the ability to escape from wild battles is disabled. Also makes Roar/Whirlwind and Teleport (under Gen8) fail.
#define B_FLAG_AI_VS_AI_BATTLE      FLAG_SYS_AI_VS_AI_BATTLE     // If this flag is set, the player's mons will be controlled by the ai next battles.
#define B_FLAG_DYNAMAX_BATTLE       FLAG_SYS_DYNAMAX_BATTLE     // If this flag is set, the ability to Dynamax in battle is enabled for all trainers.
...
#define B_VAR_STARTING_STATUS       VAR_SYS_STARTING_STATUS     // If this var has a value, assigning a STATUS_FIELD_xx_TERRAIN to it before battle causes the battle to start with that terrain active.
#define B_VAR_STARTING_STATUS_TIMER VAR_SYS_STARTING_STATUS_TIMER     // If this var has a value greater or equal than 1 field terrains will last that number of turns, otherwise they will last until they're overwritten.
#define B_VAR_WILD_AI_FLAGS         VAR_SYS_WILD_AI_FLAGS     // If not 0, you can use this var to add to default wild AI flags. NOT usable with flags above (1 << 15)
...
#define B_VAR_SKY_BATTLE                  VAR_SYS_SKY_BATTLE     // If this var has a value, the game will remember the positions of Pokémon used in Sky Battles.
