...
#define B_STEAL_WILD_ITEMS          GEN_8 // In Gen9+, Thief and Covet steal a wild pokemon's item and send it to the bag. In Gens2-8, the stolen item would be held by the Thief/Covet user.
...
// Other battle flags
#define B_FLAG_INVERSE_BATTLE       FLAG_B_FLAG_INVERSE_BATTLE     // If this flag is set, the battle's type effectiveness are inversed. For example, fire is super effective against water.
#define B_FLAG_FORCE_DOUBLE_WILD    FLAG_B_FLAG_FORCE_DOUBLE_WILD     // If this flag is set, all land and surfing wild battles will be double battles.
#define B_SMART_WILD_AI_FLAG        FLAG_B_SMART_WILD_AI_FLAG     // If this flag is set, wild Pokémon will become smart, with all AI flags enabled.
#define B_FLAG_NO_CATCHING          FLAG_B_FLAG_NO_CATCHING     // If this flag is set, the ability to catch wild Pokémon is disabled.
#define B_FLAG_NO_RUNNING           FLAG_B_FLAG_NO_RUNNING     // If this flag is set, the ability to escape from wild battles is disabled. Also makes Roar/Whirlwind and Teleport (under Gen8) fail.
#define B_FLAG_AI_VS_AI_BATTLE      FLAG_B_FLAG_AI_VS_AI_BATTLE     // If this flag is set, the player's mons will be controlled by the ai next battles.
...
#define B_FLAG_TERA_ORB_NO_COST     FLAG_B_FLAG_TERA_ORB_NO_COST     // If this flag is set, the Tera Orb does not use up its charge upon Terastallization. In S/V, this occurs after an event with Terapagos.
#define B_FLAG_SLEEP_CLAUSE         FLAG_B_FLAG_SLEEP_CLAUSE     // If this flag is set, sleep clause is enabled; if the player / AI has already put a Pokémon on the opponent's side to sleep and it is still sleeping, another one can't be put to sleep. AI requires AI_FLAG_CHECK_BAD_MOVE to understand.
...
#define B_VAR_NO_BAG_USE         VAR_B_VAR_NO_BAG_USE     // If 1, the ability to use the bag in battle is disabled in trainer battles. If 2, it is also disabled in wild battles.
...
#define B_CATCHING_CHARM_BOOST          200        // % boost in Critical Capture odds if player has the Catching Charm.
